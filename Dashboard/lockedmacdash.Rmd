...
# 
mdall<-inner_join(mdall,zips, by = "Patient_Zip")%>% distinct(...1, .keep_all = TRUE) %>% filter(state =="TN") %>% nrow()
...
ggarrange(d1,d2,labels = c("A", "B"), ncol = 2, nrow = 1)
...
#   labs(title = 'Percent of Hospitalization for all SSTVIs or Endocarditis by quarter',
#        subtitle = 'TN Hospitals 2019 | Inpatient and Outpatient',
#        caption = 'End the Syndemic | DataLab 2022',
#        x = 'Yearly Quarter',
...
#subplot(c2,style(c1, showlegend=F),shareY=TRUE) %>% layout(title ='Trends in Hospitalization by age group')
